...
export class ProductosController {
  ...
  actualizar = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      const producto = await this.service.actualizar(usuarioId, productoId, body as ActualizarProductoDto);
  ...
  agregarImagen = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      const productoId = parseInt(req.params["productoId"] as string, 10);
      const file = (req as any).file as Express.Multer.File | undefined;
      const imagen = await this.service.agregarImagen(usuarioId, productoId, req.body as AgregarImagenDto, file);
  ...
  listarCategoriasPublicas = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      const tiendaId = parseInt(req.params["tiendaId"] as string, 10);
      responderOk(res, await this.service.listarCategoriasPorTienda(tiendaId), "Categorías de la tienda obtenidas");
    } catch (error) { next(error); }
  };
